...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    # ar_out acts as input to another affine layer and output of that affine
    # layer acts asa input to softmax loss function. 
    
    ar_out, ar_cache = affine_relu_forward(X, self.params['W1'],self.params['b1'])
    a2_out, a_cache2 = affine_forward(ar_out, self.params['W2'], self.params['b2'])
    scores = a2_out
    pass
    ############################################################################
    #                             END OF YOUR CODE                             #
    ...
    dx2, grads['W2'], grads['b2'] = affine_backward(dscores, a_cache2)
    grads['W2'] += self.reg * self.params['W2']    # regularization effect

    dx1, grads['W1'], grads['b1'] = affine_relu_backward(dx2, ar_cache)
    grads['W1'] += self.reg * self.params['W1']    # regularization effect
    # pass
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    if self.use_batchnorm:
      self.bn_params = [{'mode': 'train', 'running_mean':np.zeros(dims[i+1]), 'running_vars': np.zeros(dims[i+1])} for i in xrange(self.num_layers - 1)]
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, self.num_layers):
      ...
      if self.use_batchnorm:
        ...
        layer_out[i], cache_layer[i] = affine_norm_relu_forward(layer_out[i-1], w,b,gamma,beta,bn_param)
    ...
    scores, cache_lastlayer = affine_forward(layer_out[self.num_layers - 1],
    ...
    for i in reversed(xrange(1, self.num_layers)):
      ...
      if self.use_batchnorm:
        dh,dw,db,dgamma,dbeta = affine_norm_relu_backward(dx[i+1], cache_layer[i])
        dx[i] = dh
        grads['W%d' % i] = dw
        grads['b%d' % i] = db
